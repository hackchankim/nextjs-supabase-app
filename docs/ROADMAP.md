...
- **Task 009-1: 최소 참가 인원 5명으로 하향** - 우선순위
  ...
  - `lib/game/utils.ts`의 `getRoleDistribution` 공식(`heretic=floor(n/5)`, 이단대장·목사님·권사님 각 1, 장로님 n<13?1:2, 나머지 성도)은 **수정하지 않는다** — `MIN_PLAYERS`/`MAX_PLAYERS` 상수를 그대로 참조해 클램프하므로, 상수만 바뀌면 5~9명에 자동으로 올바르게 일반화된다(5명: 이단1·이단대장1·목사님1·장로님1·권사님1·성도0 → 이단 팀 2명 vs 선 팀 3명, 총원도 정확히 5로 일치).
  - `app/game/admin/page.tsx`(`canStartGame` 조건, 인원 표시 문구)와 `app/game/waiting/page.tsx`(인원 표시 문구)는 이미 `MIN_PLAYERS` 상수를 참조하므로 코드 수정 없이 상수 변경만으로 자동 반영됨.
  - `docs/PRD.md`의 "10-20명" 표기는 이미 "5-20명"으로 반영됨(선행 완료).
  - DB 스키마 변경 없음(`MIN_PLAYERS`는 앱 상수일 뿐 DB에 저장되지 않음).

  ### 테스트 체크리스트
  - [ ] 참가자 5명으로 진행자 대시보드에 [게임 시작] 버튼이 노출되고 정상 시작되는가
  - [ ] 참가자 4명으로는 여전히 게임 시작이 거부되고 에러 메시지에 "최소 5명"이 표시되는가
  - [ ] 5~9명 각각 역할 배분 시 총원이 실제 인원수와 정확히 일치하고, 이단 대장·목사님·권사님이 각 1명씩 존재하는가
  - [ ] 5명 구성(성도 0명)에서도 낮 투표·밤 행동·승리 판정이 정상 동작하는가
  - [ ] 대기실·진행자 대시보드의 "최소 N명" 안내 문구가 5로 올바르게 표시되는가
  - [ ] 기존 10명 이상 시나리오에 회귀가 없는가
